-- ============================================================
-- TAL Migration: complete_daily_workout RPC Stored Function (HOTFIXED)
-- Path: supabase/migrations/20260629_003_complete_daily_workout.sql
-- ============================================================

CREATE OR REPLACE FUNCTION public.complete_daily_workout(
    p_player_id UUID,
    p_card_id TEXT,
    p_today_kst DATE,
    p_kst_idx INTEGER
...
BEGIN
    -- 1. [HOTFIX] 하루 완주 중복 보상 판별 (Attempts Log에 DAILY_WORKOUT_COMPLETE가 오늘 KST로 이미 들어있는가?)
    SELECT EXISTS (
        SELECT 1 FROM public.speak_attempts_log
        WHERE player_id = p_player_id
          AND clip_id = 'DAILY_WORKOUT_COMPLETE'
    ...
    -- 3. 하루 끝내기 완주 XP 보상 (+100 XP) 및 스트릭 가중치 보너스 XP 정산
    v_xp_gained := 100;
    
    IF v_streak_days >= 30 THEN
        v_bonus_xp := 50; -- 30일 이상 연속 완료 보너스
    ELSIF v_streak_days >= 7 THEN
        v_bonus_xp := 20; -- 7일 이상 연속 완료 보너스
    END IF;

    v_xp := v_xp + v_xp_gained + v_bonus_xp;
    ...
    -- 5. attempts_log에 하루 끝내기 완료 기록 적재 (중복 방지 증거용)
    INSERT INTO public.speak_attempts_log (player_id, clip_id, passed, created_at)
    VALUES (p_player_id, 'DAILY_WORKOUT_COMPLETE', TRUE, now());

    RETURN jsonb_build_object(
        'success', TRUE,
        'alreadyCompleted', FALSE,
        'xpGained', v_xp_gained + v_bonus_xp,
        'currentXp', v_xp,
        'level', v_level,
        'xpToNext', v_xp_to_next,
        'levelUp', v_level_up,
